          ...
          <li key={stage.id} className="flex flex-1 items-center gap-2">
            <span
              className={cn(
                ...
                isCurrent && 'bg-brand-tint text-brand-strong ring-2 ring-brand/30',
                !isComplete && !isCurrent && 'bg-surface-sunk text-muted',
              )}
            >
              {isComplete ? <CheckIcon className="h-3.5 w-3.5" /> : index + 1}
            </span>
            <span className={cn('text-sm font-semibold', isCurrent || isComplete ? 'text-ink' : 'text-muted')}>{stage.label}</span>
            {index < journeyStages.length - 1 ? (
              <span aria-hidden className={cn('mx-1 h-0.5 flex-1 rounded-full', isComplete ? 'bg-brand' : 'bg-line')} />
            ) : null}
          </li>
        );
      })}
